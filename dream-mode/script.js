...
document.addEventListener('click', e => {
    ...
    if (
        ...
        let textToCopy = discord;

        if (target.classList.contains('mode-date') || target.classList.contains('countdown')) {
            const scheduleItem = target.closest('.schedule-item');
            if (scheduleItem) {
                const modeNameEl = scheduleItem.querySelector('.mode-name');
                if (modeNameEl) {
                    const modeName = modeNameEl.textContent.trim();

                    if (target.classList.contains('mode-date')) {
                        // mode-date: "**ModeName - timestamp**"
                        textToCopy = `**${modeName} - ${discord}**`;
                    } else {
                        // countdown: "**ModeName timestamp**"
                        textToCopy = `**${modeName} ${discord}**`;
                    }
                }
            }
        }
        // next-mode-date: just copy timestamp (no mode name)
